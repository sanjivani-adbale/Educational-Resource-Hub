...
export class AttendanceService {
  ...
  submitAttendance(attendanceRecord: AttendanceRecord): Observable<any> {
    const url = 'http://localhost:8080/attendance/take-attendance';
    return this.http.post(url, attendanceRecord);
  }

  showAllAttendance(): Observable<any[]> {
    const url = 'http://localhost:8080/attendance/get-all-attendance-records';
  ...
  searchAttendanceByDate_Subject(
    ...
    const url = `http://localhost:8080/attendance/get-attendance-by-date-subjet/${date}/${subjectId}`;
    return this.http.get<any[]>(url);
  }
}
